...
// -------------------------------------------------------------
// Avatar Picker Dialog (Built-in Avatars)
// -------------------------------------------------------------
class AvatarPicker extends StatelessWidget {
  final Function(String) onSelected;
  final String currentAvatar;
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('Choose Built-in Avatar'),
      content: SizedBox(
        width: 300,
        height: 400,
        child: GridView.builder(
...
// -------------------------------------------------------------
// Profile Picture Selection Bottom Sheet
// -------------------------------------------------------------
class ProfilePicturePickerSheet extends StatelessWidget {
  final VoidCallback onPickFromGallery;
  final VoidCallback onTakePhoto;
  final VoidCallback onPickBuiltIn;
...
// -------------------------------------------------------------
// EditProfilePage with Both Options
// -------------------------------------------------------------
class EditProfilePage extends StatefulWidget {
  const EditProfilePage({super.key});
...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  // Profile picture: can be asset path (built-in) or Firebase Storage URL
  String _selectedAvatarPath = '';
  ...
  void _showProfilePictureOptions() {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => ProfilePicturePickerSheet(
        ...
        onTakePhoto: _takePhotoFromCamera,
        onPickBuiltIn: _showBuiltInAvatarPicker,
      ),
    );
  }
  ...
  Future<void> _pickImageFromGallery() async {
    ...
    if (pickedFile != null) {
      await _uploadToFirebaseStorage(File(pickedFile.path));
    }
  }
  ...
  // ============================================================
  // ✅ ঠিক করা Upload to Firebase Storage Function
  // ============================================================
  Future<void> _uploadToFirebaseStorage(File imageFile) async {
    setState(() => _isUploadingImage = true);
    try {
      // প্রথমে চেক করুন ইউজার লগইন আছে কিনা
      final currentUser = FirebaseAuth.instance.currentUser;
      if (currentUser == null) {
        throw Exception('Please login first to upload image');
      }
      
      print('✅ Uploading for user: ${currentUser.uid}');
      
      // ফাইলের নাম তৈরি করুন
      final String fileName = 'profile_pics/${currentUser.uid}.jpg';
      final Reference ref = FirebaseStorage.instance.ref().child(fileName);
      
      // Metadata তৈরি করুন
      final metadata = SettableMetadata(
        contentType: 'image/jpeg',
        customMetadata: {'uid': currentUser.uid, 'uploadedAt': DateTime.now().toString()},
      );
      ...
      setState(() {
        _selectedAvatarPath = downloadUrl;
        _isUploadingImage = false;
      });

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('✅ Profile picture uploaded! Save your profile.')),
        );
      }
    } catch (e) {
  ...
  // Option 3: Built-in Avatars
  void _showBuiltInAvatarPicker() {
    showDialog(
      context: context,
      builder: (context) => AvatarPicker(
        currentAvatar: _selectedAvatarPath,
        onSelected: (String avatarPath) {
          setState(() {
            _selectedAvatarPath = avatarPath;
          });
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Built-in avatar selected! Don\'t forget to save.')),
          );
        },
      ),
    );
                        ...
                        children: [
                          Container(
                            ...
                            child: CircleAvatar(
                              radius: 55,
                              backgroundColor: const Color(0xFFF4F5F6),
                              backgroundImage: _selectedAvatarPath.isEmpty
                                  ? null
