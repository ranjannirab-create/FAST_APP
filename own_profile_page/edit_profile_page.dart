...
// -------------------------------------------------------------
// Avatar Picker Widget (Unchanged Logic - Premium UI)
// -------------------------------------------------------------
...
// -------------------------------------------------------------
// EditProfilePage – Premium Glass/Box Minimal UI
// -------------------------------------------------------------
...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  // ----------------------------------------------------------------------
  // DATE OF BIRTH & AGE (Logic Unchanged)
  // ----------------------------------------------------------------------
  ...
  // ----------------------------------------------------------------------
  // HOBBY SELECTION DIALOG (Logic Unchanged - Premium UI)
  // ----------------------------------------------------------------------
  ...
  // ----------------------------------------------------------------------
  // PREMIUM PRIVACY SELECTOR WIDGET
  // ----------------------------------------------------------------------
  ...
  // ----------------------------------------------------------------------
  // PREMIUM COMPONENT CONTAINER BUILDER (Box Widget)
  // ----------------------------------------------------------------------
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFFDFEFF), // আল্ট্রা ক্লিন হোয়াইট ব্যাকগ্রাউন্ড
      appBar: AppBar(
              ...
              child: Form(
                ...
                child: Column(
                  children: [
                    ...
                    // --- SECTION 2: CURRENT LOCATION BOX ---
                    _buildSectionBox(
                      children: [
                        Row(
                    ...
                    // --- SECTION 5: HOBBIES BOX ---
                    _buildSectionBox(
                      children: [
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            const Text('Hobbies', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87)),
                            TextButton.icon(
                              onPressed: _openHobbySelector,
                              icon: Icon(Icons.add, size: 16, color: primaryColor),
                              label: Text('Select (${_selectedHobbies.length}/6)', style: TextStyle(color: primaryColor, fontWeight: FontWeight.w600)),
                            ),
                          ],
                        ),
                        const SizedBox(height: 8),
                        if (_selectedHobbies.isEmpty)
                          const Padding(
                            ...
                            child: Text('No hobbies selected. Tap "Select" to add.', style: TextStyle(color: Colors.black38, fontSize: 13)),
                          ),
                        Wrap(
                          spacing: 8,
                          runSpacing: 4,
                          children: _selectedHobbies.map((h) => Chip(
                            label: Text(h, style: TextStyle(color: primaryColor, fontWeight: FontWeight.w500, fontSize: 13)),
...
// আগের সব import ও AvatarPicker অপরিবর্তিত থাকবে
...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  List<String> _selectedInterests = [];   // <<--- পরিবর্তন
  ...
  // ----------------------------------------------------------------------
  // LOAD DATA (Interests ফিল্ড 'interests' নামে সেভ হবে)
  // ----------------------------------------------------------------------
  Future<void> _loadUserData() async {
    setState(() => _isLoading = true);
    try {
  ...
  // Location, Birthday, Age calculation methods same as before
  // ( _getCurrentLocation, _selectBirthday, _calculateAge অপরিবর্তিত )

  // ----------------------------------------------------------------------
  // INTEREST SELECTOR DIALOG (পূর্বের Hobby selector-এর নাম পরিবর্তন)
  // ----------------------------------------------------------------------
  Future<void> _openInterestSelector() async {
    List<String> tempSelected = List.from(_selectedInterests);
    await showDialog(
  ...
  // Privacy selector widgets same as before

  // ----------------------------------------------------------------------
  // SAVE PROFILE (interests ফিল্ডে সেভ)
  // ----------------------------------------------------------------------
  Future<void> _saveProfile() async {
    if (!_formKey.currentState!.validate()) return;
    setState(() => _isLoading = true);
    ...
    final data = {
      ...
      'classLevel': _workType == 'study' ? _classLevel : '',
      'jobCategory': _workType == 'job' ? _jobCategory : '',
      'interests': _selectedInterests,           // <<--- interests
      'birthday': _birthday?.toIso8601String(),
      'profilePic': _selectedAvatarPath,
      'agePrivacy': _agePrivacy,
      'addressPrivacy': _addressPrivacy,
  ...
  // _buildSectionBox, _buildTextField, _buildPrivacySelector ইত্যাদি অপরিবর্তিত থাকবে
              ...
              child: Form(
                ...
                child: Column(
                  children: [
                    ...
                    // SECTION 5: INTERESTS BOX (hobbies না হয়ে interests)
                    _buildSectionBox(
                      children: [
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        ...
                        Wrap(
                          ...
                          children: _selectedInterests.map((interest) => Chip(
                            ...
                            side: BorderSide(color: primaryColor.withOpacity(0.15)),
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                            onDeleted: () => _removeInterest(interest),   // interest variable ঠিক আছে
                          )).toList(),
                        ),
                      ],
                    ),
  ...
  _buildSectionBox({required List<RenderObjectWidget> children}) {}
}
*/
import 'package:flutter/material.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  Future<void> _saveProfile() async {
    ...
    final data = {
      ...
      'classLevel': _workType == 'study' ? _classLevel : '',
      'jobCategory': _workType == 'job' ? _jobCategory : '',
      'interests': _selectedInterests,   // <-- Fixed: was 'hobbies'
      'birthday': _birthday?.toIso8601String(),
      'profilePic': _selectedAvatarPath,
      'agePrivacy': _agePrivacy,
      'addressPrivacy': _addressPrivacy,
  ...
  // ----------------------------------------------------------------------
  // PREMIUM COMPONENT CONTAINER BUILDER
  // ----------------------------------------------------------------------
  Widget _buildSectionBox({required List<Widget> children}) {
    return Container(
      width: double.infinity,
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            const Text('Interests', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87)),
                            TextButton.icon(
                              onPressed: _openInterestSelector,   // <-- fixed method name
                              icon: Icon(Icons.add, size: 16, color: primaryColor),
                              label: Text('Select (${_selectedInterests.length}/6)', style: TextStyle(color: primaryColor, fontWeight: FontWeight.w600)),
                            ),
                          ],
